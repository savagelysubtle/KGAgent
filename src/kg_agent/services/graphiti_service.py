...
class GraphitiService:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            # Query Neo4j directly for stats using the driver
            ...
            # Result is a list of records, each record is a dict
            node_count = 0
            if node_result and len(node_result) > 0:
                ...
                if isinstance(record, dict):
                    ...
                    node_count = record.data().get("count", 0)
            ...
            if edge_result and len(edge_result) > 0:
                record = edge_result[0]
                if isinstance(record, dict):
                    edge_count = record.get("count", 0)
                elif hasattr(record, "data"):
                    edge_count = record.data().get("count", 0)
            ...
            if episode_result and len(episode_result) > 0:
                record = episode_result[0]
                if isinstance(record, dict):
                    episode_count = record.get("count", 0)
                elif hasattr(record, "data"):
                    episode_count = record.data().get("count", 0)
